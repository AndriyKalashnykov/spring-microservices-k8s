...
| 5 | Migrate Dockerfiles from `-Djarmode=layertools extract` to `-Djarmode=tools extract --layers --launcher` | Needs scoped change | Spring Boot deprecated the `layertools` jarmode and emits a warning on every image build. The replacement `tools` jarmode is not a drop-in: it extracts into a nested `app/` subdirectory by default (vs. directly into CWD), so the four COPY steps (`/tmp/dependencies/`, `/tmp/snapshot-dependencies/`, `/tmp/spring-boot-loader/`, `/tmp/application/`) and the `JarLauncher` ENTRYPOINT all need to be updated together. A naïve one-line change was attempted (commit `5d2edfe`, reverted) and broke all four image builds. Plan the migration as a single PR that updates the extract command, the COPY paths, and verifies via `make image-build` + `make e2e`. |

...
|---|------|--------|-----------------------------------------|-------|
| 1 | Drop `tools.jackson.core:jackson-core` override | ✅ DONE 2026-06-13 | — | Override **removed** from all 4 module poms. The Boot 4.1.0 BOM ships jackson-core `3.1.4` (≥ the `3.1.1` fix floor for **GHSA-2m67-wjpj-xhg9**), so the pin was redundant; jackson-core now tracks the BOM cleanly (no core/databind skew). The matching `tools.jackson.core:jackson-core` entry was removed from renovate.json's CVE-override `automerge:false` rule. This supersedes Renovate PR #82 (which would have bumped only the override to 3.2.0, re-introducing a skew). |
| 1b | Drop `org.apache.tomcat.embed:tomcat-embed-core` override | BOM parity reached — safe to remove | Boot 4.1.0 BOM ships `<tomcat.version>` = `11.0.22`, identical to the override — verify via the same SB 4.1.0 BOM lookup | Module poms pin `tomcat-embed-core:11.0.22` to fix **CVE-2026-34483 / -34486 / -34487** (all HIGH). As of Boot 4.1.0 the BOM ships `11.0.22` (parity, and would also bring `tomcat-embed-el`/`-websocket` to 11.0.22), so the override block can be deleted from all 4 module poms in a focused cleanup PR. |
...
| 4 | Spring Boot 4.0 → 4.1 migration | ✅ DONE 2026-06-13 | — | Landed `spring-boot-starter-parent 4.1.0` + `spring-cloud-dependencies 2025.1.2` together (the only supported pairing — Spring Cloud 2025.1.2 / commons 5.0.2 added `compatible-boot-versions = 4.0.x, 4.1.x`; 4.1.0 + 2025.1.1 is rejected by the CompatibilityVerifier, and 2025.1.2 + 4.0.6 fails to compile because 2025.1.2 requires Boot ≥ 4.0.7 and drops the transitive `spring-boot-restclient` edge). Also added an explicit `spring-boot-restclient` dependency to department + organization (they import `RestClientBuilderConfigurer` directly). Verified locally: build, lint (warnings-as-errors), unit, and integration tests all green. No new deprecation warnings. |
| 5 | MongoDB 8.x cadence — pin `mongo:8.0` LTS or keep riding the quarterly line | Decision pending | - Renovate opens a `mongo:8.4.x` PR (next quarterly-line bump)<br>- The current `8.3` line nears its EOL | `k8s/mongodb-deployment.yaml` now pins `mongo:8.3.4@sha256:…` — Renovate has already moved it 8.2.7 → 8.3.1 → 8.3.2 → 8.3.4, so the earlier "8.2 → 8.3 bump" trigger has fired. MongoDB quarterly "rapid release" lines are short-lived; **`mongo:8.0`** is the LTS line (EOL **2029-10-31**). Two paths: (a) re-pin to `mongo:8.0` LTS — less churn, always supported (then update Renovate to follow `8.0.*` only); (b) keep accepting the 8.x quarterly cadence and let Renovate keep us current (more churn, always near a line EOL). Decision is style — both are safe. The `MongoDB` Renovate group (added 2026-05-22) now lands the manifest + Testcontainers + docs bumps in one PR. |

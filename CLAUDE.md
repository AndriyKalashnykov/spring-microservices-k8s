...
|---|------|--------|-----------------------------------------|-------|
| 1 | Drop `tools.jackson.core:jackson-core` override | BOM parity reached — safe to remove (coordinate with the in-flight jackson-monorepo Renovate PR) | Boot 4.1.0 BOM ships `<jackson-bom.version>` = `3.1.4`, identical to the override — verify via `repo1.maven.org/maven2/org/springframework/boot/spring-boot-dependencies/4.1.0/spring-boot-dependencies-4.1.0.pom` | Module poms pin `tools.jackson.core:jackson-core:3.1.4` in `<dependencyManagement>` to fix **GHSA-2m67-wjpj-xhg9** (HIGH — Document length constraint bypass). As of Boot 4.1.0 the BOM ships `3.1.4` (parity), so the override is now removable. Hold until the jackson-monorepo Renovate PR (which bumps the override ahead of the BOM) settles, then delete the `<dependency>` block from all 4 module poms if the BOM still covers the fix floor. |

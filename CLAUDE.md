...
|---|------|--------|-----------------------------------------|-------|
| 1 | Drop `tools.jackson.core:jackson-core` override | Blocked on Spring Boot 4.0.6 | `mvn dependency:tree -Dincludes=tools.jackson.core:jackson-core` shows natural version ≥ 3.1.1 (Renovate bumped Spring Boot) | Module poms pin `tools.jackson.core:jackson-core:3.1.1` in `<dependencyManagement>` to fix **GHSA-2m67-wjpj-xhg9** (HIGH — Document length constraint bypass). Spring Boot 4.0.5 manages `jackson-core:3.1.0` which is vulnerable. Remove the override once SB 4.0.6 (or later) ships with 3.1.1 managed; Renovate should flag it via the Spring Boot group rule. |
